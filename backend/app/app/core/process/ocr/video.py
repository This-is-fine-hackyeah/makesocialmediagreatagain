...
def _ocr(frame, lang):
    ...
    text = tesserocr.image_to_text(pil_image, lang=lang)
    frame.text = text
    pbar.update()
    return frame


def _parallel_ocr(frames, lang):
    with ThreadPool(multiprocessing.cpu_count()) as pool:
        return pool.map(lambda a: _ocr(a, lang=lang), frames, chunksize=multiprocessing.cpu_count())
...
def ocr_video(filepath: str, sample_rate: int = 1, debug_dir: str = "", lang: str = "pol"):
    f"""
    ...
    with _open_cv_video(filepath) as cap:
        frames = _parallel_ocr(_filter_redundant_frames(_get_frames(cap, sample_rate)), lang=lang)
    ...
    return [f.text for f in non_empty_frames]
